  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      ...
      {!isInstalled && !installDismissed && installPrompt && (
        <div className="fixed bottom-16 left-3 right-3 z-40 bg-gray-900 border border-gray-700 rounded-2xl px-4 py-3 flex items-center gap-3 shadow-2xl animate-slide-up">
          <div className="text-3xl shrink-0">📲</div>
          <div className="flex-1 min-w-0">
            <p className="text-white font-bold text-sm leading-none">Instala la app</p>
            <p className="text-gray-400 text-xs mt-0.5">Accede rápido desde tu pantalla de inicio</p>
          </div>
          <button onClick={instalarApp}
            className="bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-xl text-xs font-bold shrink-0 transition-colors">
            Instalar
          </button>
          <button onClick={dismissInstall} className="text-gray-500 hover:text-gray-300 text-lg leading-none shrink-0">×</button>
        </div>
      )}
...
function SocialFooter({ config }: { config: { redes_sociales: RedSocial[]; direccion_local: string; whatsapp_phone: string } }) {
  ...
  return (
    <div className="border-t border-gray-800/50 mt-6 pt-5 pb-2 text-center space-y-3">
      ...
      <p className="text-gray-700 text-xs">Contry Burger · {new Date().getFullYear()}</p>
    </div>
  )
}
